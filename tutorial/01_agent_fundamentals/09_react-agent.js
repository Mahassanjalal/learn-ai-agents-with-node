...
const systemPrompt = `You are a strict step-by-step mathematical reasoning assistant. 
Always follow this pattern:

1. Thought: Explain your next calculation.
2. Action: Call ONE of these tools: add(a,b), subtract(a,b), multiply(a,b), divide(a,b)
3. Observation: Wait for the tool result.
4. Repeat until you have the final answer.
5. Answer: Only write the final answer when done.

Rules:
- Never do math in your head; always use tools.
- Only call one tool per Action.
- Stop immediately after giving the Answer.

Example:
User: "What is 5 + 3, then multiply by 2?"
Thought: First, I need to add 5 and 3
Action: add(5,3)
Observation: 8
...
// optimized ReAct Agent execution loop
async function reactAgent(userPrompt, maxIterations = 10) {
    console.log("\n" + "=".repeat(70));
    console.log("USER QUESTION:", userPrompt);
    console.log("=".repeat(70) + "\n");
    ...
    let fullResponse = "";
    let promptForIteration = userPrompt;

    for (let iteration = 1; iteration <= maxIterations; iteration++) {
        console.log(`--- Iteration ${iteration} ---`);

        let currentChunk = "";
        const response = await session.prompt(promptForIteration, {
            functions,
            maxTokens: 300,
            onTextChunk: (chunk) => {
                process.stdout.write(chunk);
        ...
        // If final answer detected, stop
        if (/answer:/i.test(fullResponse)) {
            console.log("\n" + "=".repeat(70));
            console.log("FINAL ANSWER REACHED");
            console.log("=".repeat(70));
            return fullResponse;
        ...
        // Feed last chunk back into prompt
        promptForIteration = "Continue reasoning step-by-step using tools. Last output:\n" + currentChunk;
    }
